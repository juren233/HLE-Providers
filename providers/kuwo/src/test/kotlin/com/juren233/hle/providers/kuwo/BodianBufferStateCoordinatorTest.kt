...
class BodianBufferStateCoordinatorTest {
    @Test
    fun `freezes at the real playing position when buffer starts`() {
        val coordinator = BodianBufferStateCoordinator()
        assertEquals(BodianPlaybackDecision.Forward, coordinator.onPlaybackState(playing()))

        val decision = coordinator.onBufferStarted(nowMs = 12_500L)
            as BodianPlaybackDecision.Publish
    ...
    fun `pause cancels the synthetic buffer and late end is ignored`() {
        ...
        assertEquals(BodianPlaybackDecision.Ignore, coordinator.onBufferEnded(42_500L))
    }
    ...
    fun `buffer callbacks do not synthesize playback without a real playing state`() {
        ...
        assertEquals(BodianPlaybackDecision.Ignore, coordinator.onBufferStarted(12_500L))
        assertEquals(BodianPlaybackDecision.Ignore, coordinator.onBufferEnded(13_000L))
        assertTrue(coordinator.onPlaybackState(null) is BodianPlaybackDecision.Forward)
    }
