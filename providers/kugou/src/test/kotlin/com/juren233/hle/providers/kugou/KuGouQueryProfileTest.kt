...
class KuGouQueryProfileTest {
    ...
    fun `next track profiles preserve the verified semantics of each Kugou variant`() {
        ...
        assertNull(full[1].preferredTarget)
        assertEquals("kugou-full-next-media-v3", full[1].cacheKey)
        assertEquals(listOf("getNextMedia"), full[1].requiredCallerMethodNames)
