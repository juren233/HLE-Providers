...
- 2026-09-06 23:30 用户复测核心150255/插件19仍有息屏切歌进度异常。新PID11154明确自动锚点0接受、插件writer关闭、无legacy_boolean覆盖，但核心screen_off跳过ticker后订阅者持续35163ms，新歌63行在AOD按约35秒选句。撤回插件自动锚点优先足以解决全部复现的结论，下一断点位于核心锚点发布，不重复修改插件回退选择。

## KUGOU-SEARCH-001: 旧版概念版无 mediaId 时双语关键词零召回且无回退

- 状态：修复中，等待真机验收（issue #16 报告者环境，维护者设备无法直接复现）。
- 症状与复现：报告者（小米 10 Pro，HyperOS 4.0.0.25 移植系统界面，酷狗概念版 `2.5.5` 32 位）播放时超级岛只有"标题 - 歌手"单行占位，无歌词与翻译；同一首歌在维护者设备（酷狗概念版 `5.2.7`）歌词与翻译正常。
- 可靠运行证据：报告者模块导出日志中酷狗 `central_song_callback` 的 songId 全部为 64 位 sha256（`KuGouTrackMetadata.identity` 在 mediaId 缺失时退化为 `sha256(title\0artist\duration)`）；维护者设备同曲 songId 为数字 `64540172`（MediaSession 提供 mediaId=album_audio_id）。32 位进程 dlopen armeabi-v7a libdexkit 失败只影响下一首预览，与歌词无关。模块日志导出只含 LSPosed 标签，Provider 自身日志（TAG=`HLEProvider/KuGou`）需 `adb logcat -s HLEProvider/KuGou` 单独采集。
- 实测 API 证据：v2/search 用"双语标题"关键词（album_audio_id=0、hash 空，即报告者形态）返回 errcode=200 且 0 候选；去掉括号段重搜同一首歌得 10 候选，胜出 121 分=完整双语标题精确 65+歌手 30+时长 20+服务器 6，证明酷狗库存标题本就是完整双语名，失败仅在服务器关键词召回；带 album_audio_id 时同一关键词可得 17 候选（服务器按 ID 匹配、容忍关键词）。
- 已证伪方向：(1) "在线翻译白名单缺酷狗包"——酷狗翻译来自 Provider 自身 v2 API，白名单按设计只含 Apple/汽水/Spotify/椒盐；(2) "双语括号关键词普遍搜不到"——维护者设备同曲正常，决定因素是 mediaId 有无，不是关键词本身。
- 修复边界：搜索改阶梯式——完整关键词优先；仅当该请求成功但无可用候选（0 候选或全部低于 MINIMUM_SCORE）时，才用去掉括号段的标题变体（保留歌手前缀）重试，变体封顶 3 个；采纳标准不变，候选仍按完整原始标题/歌手/时长打分且 ≥70。发布的歌名与歌手永远取 MediaSession 元数据（`placeholder`/`toSong`），搜索词不影响显示名。服务器报错（errcode≠200）中断阶梯不重试。不打包 armeabi-v7a dexkit，旧版 2.5.5 以升级引导为主。
- 当前未知：报告者设备安装含阶梯搜索的 Provider 后双语歌曲能否实际出歌词；32 位旧版环境下 v2 API 是否有其他差异。
...
- 当前未知：a74 `a()`/`b()` 哪个对应 enable_v3（消费方走接口分发）；链式注解锚在 9.1.72/9.1.80 之外的版本（如报告者 9.1.68.1888）上的结构是否同构，未做 DEX 取证前不预设。
...
- 候选交付（2026-09-07，持久化腿）：core 主模块查询 schema 增加 `requiredMethodAnnotation`（只锚注解元素值，注解类型名/元素名可空——R8 每版改名注解类：9.1.72=p.thy、9.1.80=p.vsz）与 `declaringClassFieldTypeNames`/`declaringClassFieldReferences`（声明类持有某类型字段约束），`OfficialProviderConstructorTarget.firstParameterTypeName` 支持按首参唯一匹配构造器；查询主构造器冻结，新增 Builder 作为唯一扩展点，历史 JVM 签名由 hidden 构造器保留并配描述符锁定测试。provider 1.0.13 (14) 新增 `lyricsChainQueries`（v2/v3 服务注解锚查询 + 引用服务声明类字段的包装类 `b(String,String)Single` 查询，服务先于包装）与 `chainProfile`（解析结果映射为运行期档案，构造器目标只带服务首参约束）；PluginEntry 未知版本进入链式解析（不再依赖精确档案，同键目标自动去重），`minCoreVersionCode=151009`。取证补充（两版 DEX 探针复核）：服务接口结构对称（v3 的 `b`=4 参 track 端点、v2 的 `a`=4 参 track 端点），track 端点注解值全 App 唯一；服务字段持有者唯一（9.1.72 `xl80` 另有 MessageRequest 持有但无 `b(String,String)Single`，不构成歧义）。核心单测与 pack 51 项单测全绿。真机验收要点（未知版本，如 9.1.68.1888）：`Spotify 版本 ... 进入注解锚链式解析` → `Spotify 链式解析成功: v3=..., v2=...` → `歌词客户端构造 Hook 首次命中` → 主动兜底 `Spotify color-lyrics 异步成功首次命中: lines=N`；9.1.80/9.1.72 走精确档案快路径不受影响。
- SPOTIFY-LYRICS-001 禁令“不得调整同一请求延迟”的新前提（2026-09-08）：旧延迟调优是在“客户端交付被观测门禁卡死”的前提下做无效调整；1.0.15 (16) 落地构造捕获默认交付（无观测时按线上默认 v2 交付，权威观测到达后覆盖）后，后台主动兜底真正可发起，延迟才成为拿词延迟参数。据此按维护者指示把 `LYRICS_FALLBACK_DELAY_MS` 1200ms→200ms。待验证未知：歌词客户端构造 Hook 在冷启动后台（不打开歌词页）是否命中（DI 急切 vs 懒构造），发布后以「构造 Hook 首次命中 + 内部接口歌词请求开始」日志判定；若懒构造则兜底仍只能等首次构造，且按禁令不得自建 HTTP。
